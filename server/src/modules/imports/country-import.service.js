...
function sourceFrom(record = {}) {
  ...
  return {
    source_name: pick(source, ['source_name', 'name'], pick(record, ['source_name'], null)),
    source_link: pick(source, ['source_link', 'link', 'url'], pick(record, ['source_link'], null)),
    notes: pick(source, ['notes'], pick(record, ['source_notes', 'notes'], null))
  };
}

function normalizeCountryRecord(record) {
  const countryName = String(pick(record, ['country_name', 'name', 'country'], '')).trim();
  const isoCode = pick(record, ['iso_code', 'iso', 'country_code'], null);
  return {
    ...record,
...
function yearlyMetricsFrom(record) {
  ...
  return extractYearlyMetrics(record).map((item) => ({
    financial_year: item.financial_year,
    rank: item.rank ?? null,
    export_value_usd_mn: item.export_value_usd_mn ?? null,
    yoy_growth_percent: item.yoy_growth_percent ?? null,
    three_year_cagr_percent: item.three_year_cagr_percent ?? null,
    share_in_india_total_exports_percent: item.share_percent ?? null,
    opportunity_score: parseNumber(pick(record, ['opportunity_score'], null)),
    risk_score: parseNumber(pick(record, ['risk_score'], null))
  }));
}
...
function latestCountryMetric(record, yearlyMetrics) {
  ...
  return {
    financial_year: normalizeFinancialYear(pick(latest, ['financial_year', 'fy'], pick(record, ['financial_year'], null))),
    rank: parseNumber(pick(latest, ['rank'], pick(record, ['rank'], null))),
    ...
    computed_share_in_india_exports_percent: parsePercent(
      pick(latest, ['computed_share_in_india_exports_percent'], pick(record, ['computed_share_in_india_exports_percent'], null))
    ),
...
async function upsertIndiaSummary(summary, importBatchId) {
  ...
  const financialYear = normalizeFinancialYear(pick(summary, ['financial_year', 'fy'], null));
  ...
  await IndiaExportSummary.findOneAndUpdate(
    { financial_year: financialYear },
    {
      $set: {
        financial_year: financialYear,
        india_total_exports_usd_bn: parseNumber(pick(summary, ['india_total_exports_usd_bn', 'total_exports_usd_bn'], null)),
        yoy_growth_percent: parsePercent(pick(summary, ['yoy_growth_percent', 'yoy_growth'], null)),
        top_export_sector: pick(summary, ['top_export_sector'], null),
        total_export_destinations: parseNumber(pick(summary, ['total_export_destinations'], null)),
        estimated_hs_codes_exported: pick(summary, ['estimated_hs_codes_exported'], null),
        is_partial_year: Boolean(pick(summary, ['is_partial_year'], false)),
        source,
  ...
  await CountryBusinessInsight.findOneAndUpdate(
    { country_id: country._id },
    {
      $set: {
        ...
        source: {
          source_name: pick(insights.source || {}, ['source_name'], pick(record, ['source_name'], null)),
          source_link: pick(insights.source || {}, ['source_link'], pick(record, ['source_link'], null))
        },
        import_batch_id: importBatchId
      }
    },
...
async function upsertFastestProfile(country, record, importBatchId) {
  ...
  const financialYear = normalizeFinancialYear(pick(profile, ['financial_year', 'fy'], country.latest_export_snapshot?.financial_year));
  if (!financialYear) return;
  ...
  await FastestGrowingCountryProfile.findOneAndUpdate(
    { country_id: country._id, financial_year: financialYear },
    {
      $set: {
        ...
        financial_year: financialYear,
        export_value_usd_mn: parseNumber(pick(profile, ['export_value_usd_mn', 'export_usd_mn'], null)),
        yoy_growth_percent: parsePercent(pick(profile, ['yoy_growth_percent', 'yoy_growth'], null)),
        ...
        source: {
          source_name: pick(profile.source || {}, ['source_name'], pick(record, ['source_name'], null))
        },
        import_batch_id: importBatchId
      }
    },
...
async function importOneCountry(record, importBatchId, summary) {
  ...
  const countryDoc = {
    ...
    trade_profile: {
      ...
      top_hs_chapters: splitCommaList(pick(tradeProfile, ['top_hs_chapters'], pick(normalized, ['top_hs_chapters'], null))),
